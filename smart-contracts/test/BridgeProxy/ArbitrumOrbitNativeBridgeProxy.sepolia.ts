...
import { Outbox } from '@relay-protocol/helpers/abis'
...
describe('ArbitrumOrbitNativeBridgeProxy', function () {
  ...
  describe('claim', () => {
    ...
    it('works using ERC20 (WETH)', async () => {
      ...
      const iface = new ethers.Interface(Outbox)
      ...
      expect(event.args.to).to.equals(returnedProof.destination)
      expect(event.args.l2Sender).to.equals(returnedProof.caller)
      expect(balanceAfter).to.equals(
        balanceBefore + ethers.parseEther('0.000001')
      )

      // TODO: clain using relay pool
