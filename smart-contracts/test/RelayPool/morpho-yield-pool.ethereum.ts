...
describe('RelayBridge: use Morpho yield pool (WETH)', () => {
  ...
  before(async () => {
    ...
    const staticAavePoolGetter = await ethers.getContractAt(
      ...
      await morphoUsdcPool.getAddress()
    )
    ...
    const parameters = {
      RelayPool: {
        ...
        origins: [],
        thirdPartyPool: await morphoUsdcPool.getAddress(),
        weth,
      },
    }
  ...
  it('should deposit tokens into the Aave V3 yield pool when a user deposits funds', async () => {
    ...
    const balanceThirdPartyPoolBefore = await usdc.balanceOf(USDC_MORPHO_POOL)
    ...
    expect(await usdc.balanceOf(USDC_MORPHO_POOL)).to.equal(
